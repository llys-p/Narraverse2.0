...
- 分工以用户当前指令为准；作品设定库后续任务、阶段依赖与勾选状态以 [交接清单](plans/LIBRARY_EVOLUTION_TASK_CHECKLIST.md) 为准，不能把骨架视为已实现功能。真实用户资料迁移仍需具体选择与确认。
...
| L2/L3/L4 资料库 | L1 补缺与 L2 只读预览已在本地验收，当前改动未提交/未部署；L3 模式接入、L4 迁移仅有骨架 | 从 [总骨架](plans/LIBRARY_EVOLUTION_BLUEPRINT.md)、[交接清单](plans/LIBRARY_EVOLUTION_TASK_CHECKLIST.md) 和 [验收报告](acceptance/LIBRARY_L1_L2_ACCEPTANCE.md) 接手；不把预览当已送模 |
