...
- 提交计数：**锚定固定区间**——核对 `git rev-list --count e570b0e..dfbda8c` **应为 18**（含复审修复轮），**不要用 `e570b0e..HEAD` 数总数**，免得 `dfbda8c` 之后的文档提交令数字漂移；`dfbda8c` 之后应仅有文档类提交（`d6badbd` 交接更正、`bf1bde8` 本提示词落档，及对提示词本身的表述修正），用 `git log --oneline dfbda8c..HEAD` 逐一核对，这一段的提交**不计入 18**；另核对上一版交接文档曾把数量写成 16、已在 `d6badbd` 更正。
