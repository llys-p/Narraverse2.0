...
| `denova-src/internal/worldcontext/` | **Phase 3.0B 新增**：World Context 纯内存派生领域核心（`types/selection/snapshot/projection/budget/registry`） | 无网络、无写盘、无模型调用；同输入必同字节；**不拥有文件路径**，Snapshot/ModelView 绝不进 World JSON |
| `denova-src/internal/app/world_context_service.go` | **Phase 3.0B P2**：App 层只读装配 | 只经既有 `GetWorld` 读 World+revision，交 `worldcontext` 校验/投影；**不写 World、不注册 HTTP 路由、不调用模型** |
| `denova-src/internal/app/task.go` | **Phase 3.0B P3**：Task 延迟启动基础（`newPendingTask`/`start`/`discard`） | `NewTask` 语义不变（创建即启动）；延迟能力仅供未来原子认领与 3.2-A/B 接线 |
| `docs/plans/WORLD_WORKSPACE_PHASE3_ARCHITECTURE_PLAN.md` | Phase 3 架构设计（**v2.7 已冻结**） | 改契约前先看 §17 裁定；v2.7 起 3.0B 设计冻结 |
| `docs/plans/WORLD_WORKSPACE_PHASE3_IMPLEMENTATION_PLAN.md` | Phase 3.0B 实施计划（P0–P5 分解与门禁） | 编码范围/停止线以此为准；**获批前不编码** |
...
### 1.15 World Workspace Phase 3.0B：World Context 只读上下文（2026-09-11）

设计冻结于 `docs/plans/WORLD_WORKSPACE_PHASE3_ARCHITECTURE_PLAN.md` **v2.7**；实施分解与门禁见 `WORLD_WORKSPACE_PHASE3_IMPLEMENTATION_PLAN.md`（P0–P5）。**3.0B 只做服务层核心，不接 HTTP、不接前端、不接 Module3/4。**

**核心定位**：从一份**已保存、revision 匹配**的 World 构建内部 Snapshot，生成 UI / Model 双投影，维护进程内 runContext 注册表。全部是**派生状态**，进程重启即空、不持久化。

**不可违背的不变量**（改这块代码前必须确认）：

- **World 是唯一持久化真源**：`cfg.DataDir()/worlds/world-<id>.json`。Snapshot / sidecar / ProjectionBody / runContext / ModelView **均不写入 World JSON**；四模式只读，运行前后 World revision 不变。
- **双投影分离**：`ProjectionBody`（source-neutral，按 fingerprint 共享、有 `refCount`）与**最终 ModelView 每 run 独占**（每 run 独立 `runSalt` → `sourceRef` → 字节必然不同）。**绝不可缓存最终 ModelView 跨 run 复用。**
- **纯函数性质**：`projection.go` 同输入必同字节；随机性**只允许**在 runContext 创建时生成 `runSalt`。sidecar 不参与 fingerprint 与任何投影。
- **冻结数值**：body ≤32 条且合计 ≤8 MiB、单 body ≤96 KiB；run ≤64 条；idle TTL 30min、absolute TTL 6h；ModelView ≤48,000 rune / ≤96 KiB / ≤12,000 token。

**三层职责边界**

| 层 | 位置 | 能做什么 | 禁止 |
|---|---|---|---|
| 领域核心 | `internal/worldcontext/` | 纯计算：selection 归一化/闭包、Snapshot、双投影、预算估算 | 无网络、无写盘、无模型调用、不拥有文件路径 |
| 注册表 | `internal/worldcontext/registry.go` | `Bind`/`Reuse`/`GetByID`/`Destroy`/`Sweep` 五态生命周期 | 不自行读写 World |
| App 装配 | `internal/app/world_context_service.go` | 经既有 `GetWorld` 读 World+revision，交领域层 | 不写 World、不注册路由、不调用模型、不把 Snapshot 放进持久配置/任务事件 |

**Task / InteractiveRun（P3 起）**：`Task` = **一次执行尝试**；`InteractiveRun ⊃ {Task#1, Task#2(regenerate), …}`。regenerate 经 `regenerate_from_turn_id` 找回同一 InteractiveRun（**复用** runContext，新建的是 Task）；**abort 只结束当前 Task**，不解绑 runContext；**普通非 regenerate 的新回合必须创建新 InteractiveRun**。P3 的 `newPendingTask()/start()/discard()` 只提供“分配 ID 尚未启动”的最小能力，`NewTask` 语义（创建即启动）**保持不变**。

**analysisHandle（P4，待实现）**：`context-analysis` → pending context → opaque handle → 首次 chat 消费 → 建运行身份 → runContext 迁移。handle **短期（≤10min，不续期）、单次消费、不进任务日志、不作长期身份**；前端只可见 `{ analysisHandle, expiresAt }`；失效一律**非阻断**（忽略 + 常规解析）。

**当前进度**：P0 领域核心、P1 两级 Registry、P2 App 只读装配、P3 Task 延迟启动 **已提交待审**；P4（analysisHandle/InteractiveRun 服务层）与 P5（错误脱敏）**未开始**。

**踩坑**：本目录 77 个 `denova-src/internal/app/*.go` 长期显示 `M`，但 `git diff --ignore-all-space` 为空——**只是 CRLF 行尾噪音，不是真实改动**。提交前务必用 `git diff --ignore-all-space --name-only` 过滤，避免误暂存。
