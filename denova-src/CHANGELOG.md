...
- 加固 B5 验收脚手架为 fail-closed（复审跟进）：一键自检不再凭端口响应判就绪（改为校验本进程 stdout 报出的监听地址、进程存活与应用特征字段），写作与游戏链分别断言 SSE 完成、工具往返与模型取材，游戏 run ledger 须存在元数据化读取事件，标记扫描器把遍历错误真正接到 `os.walk onerror`（不可读/悬空目录即失败）。负例（端口被占、悬空目录）均正确失败，全绿复跑通过。
- Hardens the B5 acceptance harness to fail closed (review follow-up): the smoke runner no longer treats a bare port response as ready (it verifies the process's own reported backend URL, its liveness and the app status signature), the writing and game drivers assert their own stream completion, tool round-trip and grounding, the game run ledger must hold a metadata-only read event, and the marker scanner actually wires the walk-error handler (os.walk onerror). Negative cases (occupied port, dangling directory) now fail correctly, and the green rerun passes.

- 新增四模式统一验收脚手架（`scripts/library-acceptance/`，L3/B5 用，全部 stdlib）：确定性假模型端点（写作纯文本 / 游戏回合脚本双模式，识别压缩请求、防跑飞）、写作与游戏链的带库回合驱动（建书/建故事/选中/发送，只走已保存库 Ref 三字段）、落盘标记直扫（允许清单 + 必备目标 + 遍历读取错误即失败，防假绿）；统一验收 runbook 见 `docs/acceptance/LIBRARY_L3_B5_RUNBOOK.md`。已用隔离 exe + 假模型冒烟跑通游戏链与写作链（游戏：回合 200、模型侧取材、48 文件零泄漏扫描 exit 0；写作：chat 200、SSE 收尾、模型输入含冻结抬头与 resident/manual 正文、23 文件零泄漏扫描 exit 0）。脚手架不消耗付费额度，正式四模式验收待叙界模块资产与真实模型轮次。
- Adds the unified four-mode acceptance harness (`scripts/library-acceptance/`, for L3/B5, all stdlib): a deterministic fake-model endpoint (writing plain-text / game turn-script modes, with compaction handling and a runaway guard), library-turn drivers for the writing and game chains (create book/story, select, send only the saved-ref triple), and a persisted-marker scanner (allow-list plus required targets plus fail-on-walk/read-errors). The unified runbook lives in `docs/acceptance/LIBRARY_L3_B5_RUNBOOK.md`. Smoke-tested through the isolated executable plus the fake model on both chains (game: turn 200, model-side grounding, 48-file scan with zero leaks, exit 0; writing: chat 200 with a clean SSE close, the model input carrying the frozen header plus resident/manual bodies, and a 23-file scan with zero leaks, exit 0). The harness costs no paid quota; the formal four-mode acceptance awaits the Narraverse module assets and real-model rounds.

- 接通作品设定库开放沙盒（Module4）链路（B4b/L3.4）：受控 iframe 宿主层放开 module4 的库载体（与叙界共用同一 bind-before-start 与 wire：严格 Ref 三字段、与 world 载体互斥、consumer 路由固定、scopeKey 服务端派生、绑定期固定 revision 与装配计费、每次 /call 前置装配背景并计费、换绑/解绑/撤销/过期幂等释放、响应与摘要脱敏；沙盒的模型模块与 Settings 不变）。库加载预览新增「带入开放沙盒」（与「带入叙界」同一边界：只交接已保存库 Ref 三字段，草稿禁用，宿主会话不可用时显式提示不写交接），经既有 `onOpenModule4` 受控入口切到叙界模式并在 iframe 内打开 Module4。同一宿主会话与 frame 下叙界/沙盒各绑各库、互不串库，解除其一不影响另一绑定。新增测试：app 2 例（module4 绑定/调用/解除、跨消费者不串库）+ 前端 4 例。正式页面验证（叙界模块资产不在本工作树）同 B4a 保留为验收缺口。
- The library now also reaches the Module4 open sandbox (B4b/L3.4): the controlled iframe host accepts the library carrier for module4 on the same bind-before-start path and wire as Narraverse (strict three-field ref, exclusive with the world carrier, route-fixed consumer, server-derived scope key, pinned revision and charged initial assembly at bind time, the assembled background prepended and charged on every /call, idempotent release on rebind/unbind/revoke/expiry, and sanitized responses and summaries; the sandbox keeps its existing model module and settings). The load preview gains a "bring into open sandbox" entry with the same saved-ref boundary as Narraverse (drafts disabled; the host-session guard refuses without a handoff) and opens through the existing `onOpenModule4` flow into the sandbox inside the iframe. Two consumers on one host session and frame keep their own libraries without crossing, and unbinding one leaves the other usable. New tests: 2 app cases and 4 frontend cases. The real-page acceptance (the Narraverse module assets are not in this worktree) stays open like B4a.

- 接通作品设定库叙界（受控 iframe）链路（B4a/L3.3）：库工作区加载预览新增「带入叙界」（与写作/游戏同一边界：只交接已保存库的 Ref 三字段，未保存草稿禁用，宿主会话不可用时显式提示不写交接）。宿主 bind/call 新增 library 载体（与 world 载体互斥，consumer 由受控路由固定，scopeKey 由宿主会话 + frame 实例服务端派生；Module4 保持 world-only，B4b 另行接入）：绑定期按 §8.2 bind-before-start 固定 revision 并装配只读背景，每次模型代理调用前置装配文本并计入运行预算；库文件逐字节不变，绑定期失败显式分类（revision_conflict/selection_invalid/budget_exceeded…）不留半绑定；响应与 iframe 摘要只含 state/libraryName/revisionLabel/selectedCount，不含库 ID、revision、scopeKey 或运行身份；换绑/解绑/撤销/过期幂等释放（替换与中止→取消，显式解绑→完成）。前端新 iframe 库交接 Provider（仅内存一次性）+ NarraverseWorkspace 按 launchedAt 裁决 world/library 互斥、首个模型请求等待 bind 完成、库 Ref 不进入模型代理请求。新增测试：app 4 例、handlers wire 校验 1 例、前端 6 例。正式页面验证（叙界模块资产不在本工作树，需 NARRAVERSE_SOURCE_DIR 同步）保留为验收缺口。
- The library now reaches Narraverse (the controlled iframe) end to end (B4a/L3.3): the load preview gains a "bring into Narraverse" entry with the same saved-ref boundary as writing/game (only libraryId + expectedRevision + manualItemIds are handed off; drafts are disabled; a missing secure host session is refused with explicit copy and no handoff), and the host bind/call wire gains a library carrier (mutually exclusive with the world carrier, consumer fixed by the controlled route, scope key derived server-side from the host session plus the frame instance; Module4 stays world-only until B4b). A library bind pins the revision and assembles the read-only background before start; every model-proxy call prepends that assembly and charges the run budget; the library file stays byte-identical, bind failures stay explicit (revision_conflict, selection_invalid, budget_exceeded, ...) without a half binding, and neither the response nor the summary posted to the iframe carries the library id, revision, scope key or run identity. Rebind, unbind, revoke and expiry release exactly once (replace/abort cancel, explicit unbind completes). Frontend adds an in-memory iframe library handoff provider and NarraverseWorkspace arbitration by launchedAt (later launch wins) with the first model request waiting for the bind and the library ref kept out of the model-proxy request. New tests: 4 app cases, 1 handler wire case, 6 frontend cases. The real-page acceptance (the Narraverse module assets are not in this worktree; sync needs NARRAVERSE_SOURCE_DIR) remains an open gap.
- 修复 B3 整体验收发现的 D1 契约违规：`read_library_item` 的 tool_result 事件在唯一脱敏点 `libraryReadToolEventData` 解析失败时回退回显原文（截断前缀与稳定错误码后的消息文本），把库条目正文前缀落盘进 run ledger 与故事 display 存档（B3c 实测两个运行与对应回合各含 ≤200 字节命中，含正文标记）。脱敏点现改为 fail-closed：任何解析失败形态只输出固定提示、字节数与白名单稳定错误码，绝不回显原始内容；成功分支改用宽松解码（只取首个 JSON 值的白名单元数据），真实链路里平台侧尾随数据不再把可解析结果误判为失败，条目元数据（itemId/name 等）恢复落账。新增 12 子例 fail-closed 单测矩阵与游戏集成回归（确定性假模型强制触发一次按需读取：模型侧确收全文；run ledger、故事存档与 Session 的标记直扫 0 命中；库文件与 revision 不变；regenerate 复用原绑定）。修复前集成链红测证据与日志留存 `artifacts/library-b3-fix/`（不入 Git）。扫描辅助函数同步收紧为目录遍历/文件读取错误即失败，防漏扫假绿（复审跟进）。
- Fix the D1 contract violation found in the B3 review: when parsing failed, the single sanitization point `libraryReadToolEventData` fell back to echoing the raw tool result (truncated prefix and the message text after a stable error code), persisting a library body prefix into the run ledger and the story display archive (two B3c runs and their turns each hit with ≤200 bytes including the body marker). The sanitizer is now fail-closed: every unparsable shape yields only a fixed notice, the measured byte count and a whitelisted stable error code, never the raw content; the success branch now uses tolerant decoding (first JSON value, whitelisted metadata only) so platform-side trailing data in the real chain no longer misclassifies a parsable result, restoring item metadata (itemId/name, ...) into the event. Adds a 12-case fail-closed unit matrix and a game integration regression (a deterministic fake model forces one on-demand read: the model receives the full body; marker scans over the run ledger, story archive and sessions find no hits; the library file and revision stay unchanged; regenerate reuses the original binding). Red-first evidence and logs kept under `artifacts/library-b3-fix/`, not committed. The scan helpers now fail on directory walk/read errors so a scan can never silently skip files (review follow-up).
- 完成 B3c 游戏正式闭环验收：在隔离正式游戏页面（分支 HEAD 构建的 executable、独立数据目录、虚构书/故事/设定库，凭据由用户本人在隔离实例设置 UI 填入且不入 Git）验证了真实模型经 ephemeral 注入取材（resident+manual 初装配、auto 条目运行时经 `read_library_item` 按需读取）、regenerate 不带 Ref 由服务端复用原运行绑定、库 revision 漂移时绑定期显式失败且零模型调用、切分支后不跨分支串库、流中途 reconnect 不重复提交；对已保存 Turn、run ledger、导演文件与服务日志做 16/24 字 n-gram 全量扫描，无设定正文的运行时副本（唯一命中为模型自身推理对背景的摘抄，属输出非副本），库文件前后逐字节不变。验收中发现并修复 B3b 跨模式交接竞态（见下条）。B3 游戏链路自此等待审查；B4a/B4b/B5 待审查通过后开始。
- B3c closes the game loop with real-model acceptance on an isolated production page (an executable built from the branch HEAD, its own data directory, fictional book/story/library, and credentials entered by the user directly into the isolated instance's settings UI, never stored in Git): verified real-model grounding through the ephemeral injection (resident+manual initial assembly, the auto item fetched at runtime via `read_library_item`), regenerate reusing the original run's binding server-side without any client Ref, an explicit bind-time failure with zero model calls after a library revision drift, no cross-branch library leak after switching branches, and a mid-stream reconnect without duplicate submission. 16/24-character n-gram scans over the persisted turn, run ledger, director files and server logs found no runtime copies of the library bodies (the only hits are the model's own reasoning quoting the background — output, not an injected copy), and the library file stayed byte-identical. A B3b cross-mode handoff race surfaced during acceptance and is fixed in the entry below. The B3 game chain now awaits review; B4a/B4b/B5 start after it passes.
- 修复 B3c 真实页面验收发现的 B3b 竞态：「带入游戏」确认并切换模式后，InteractiveLayout 挂载时的 `resetWorkspaceState()` 会瞬间把 `currentStoryId` 清成空串，StoryStage 把这一瞬时的 stageKey 变化误判为用户切故事，清掉刚写入的 pending World/Library 交接，导致首回合库背景失效（B3b 组件测试均以已有 storyId 挂载，未走到这条瞬态路径，真实浏览器才暴露）。失效分支现在只在 `storyId` 非空时生效——真实的故事切换必然带着新故事，瞬态空故事重置则保留交接。补一条失败先行的回归测试，按真实「挂载→重置→恢复」时序渲染，断言首个请求仍携带 `background_source:'library'` 与 `library_context` Ref 三字段；StoryStage 套件 53/53 通过。
- Fix the B3b race found during the B3c real-page acceptance run: after "bring into game" confirms and the mode switch mounts InteractiveLayout, `resetWorkspaceState()` briefly clears `currentStoryId` to an empty string, and StoryStage misread this transient stageKey change as a user story switch, dropping the freshly written pending World/Library handoff so the first turn lost the library background (B3b component tests mounted with a storyId already present and never exercised this transient path; only the real browser exposed it). The invalidation branch now applies only when `storyId` is non-empty — a real story switch always arrives with the new story, while the transient empty-story reset preserves the handoff. A failing-first regression test renders the real mount→reset→restore sequence and asserts the first request still carries `background_source:'library'` with the `library_context` Ref triple; the StoryStage suite passes 53/53.
- 接通作品设定库游戏前端闭环（B3b/L3.3 前端）：库工作区预览页新增「带入游戏」——在对话框内显式选择目标故事与分支（复用既有服务端选中/切分支流程，主分支缺失时自动注入），确认成功才写一次性交接并切换到游戏模式，取消/失败不写交接、错误可见且不改变游戏当前背景。游戏侧新交接 Provider 在首个普通新回合消费匹配当前故事/分支的交接，请求携带 `background_source:"library"` 与 `library_context{libraryId,expectedRevision,manualItemIds}`（每回合重发、服务端每回合重绑；regenerate 不发库字段由服务端按原运行复用）；与 World 背景双向互斥、后带入者获胜，绝不与 world_context/analysis_handle 同发；切故事/分支清除旧交接与选中背景，迟到请求不能覆盖新页面状态。新增 `library_context_state` 状态条：本地"已选择、尚未生效"+清除按钮，服务端 active 才显示"本回合已使用"、显式 none 显示未使用；stale 错误附加"此次重新生成没有执行，刷新不会恢复服务端运行记录"指引，错误可见不自动重试。库背景不进上下文分析。新增/扩展前端测试 21 例，zh/en 文案成对。
- Wire the Library into the game frontend (B3b/L3.3 frontend): the library workspace preview gains a "bring into game" action — the target story and branch are chosen explicitly in a dialog (reusing the existing server-side select/switch flow, injecting the main branch when missing), and the one-shot handoff is written only after the switch succeeds; cancel/failure writes nothing, errors stay visible, and the game's current background is untouched. A new game-side handoff provider consumes the handoff matching the current story/branch on the first normal new turn, sending `background_source:"library"` with `library_context{libraryId,expectedRevision,manualItemIds}` (resent every turn as the server rebinds per turn; regenerate sends no library fields and the server reuses the original run); it is mutually exclusive with the World background with the later launch winning, and never sent alongside world_context/analysis_handle. Switching story/branch clears the stale handoff and selection so late requests cannot overwrite the new page state. A new `library_context_state` status bar shows the local "selected, not yet applied" state with a clear button, "used this turn" only after the server reports active, and an explicit none state; stale errors append guidance that this regeneration did not run and refreshing cannot recover the lost server run, with errors visible and no automatic retry. Library backgrounds never enter context-analysis. 21 new/extended frontend tests; zh/en copy added in pairs.
- 修复 B3a 审查缺口：regenerate 找不到原 InteractiveRun 索引时（进程重启/索引过期）不再静默按 legacy 背景重生成——原本带库的回合会在无任何报错的情况下换回旧 Lore。现在 regenerate 路径在运行索引未命中或不可用时显式阻断：回滚兜底新建的运行、不启动模型，以稳定 `stale` 错误码与固定脱敏文案返回"原回合的运行背景已不可用"；registry 层"未命中即新建 bare run"的既有语义保持不变，阻断只发生在 regenerate 复用保证层。另补 `startInteractiveTask` 带库回合的端到端集成测试（真实库文件与真实互动存储 + 假流式模型端点）：覆盖最终送模（library 模式系统提示与 `read_library_item` 工具挂载、ephemeral 库正文逐字节前置、未授权条目不在）、持久化隔离（turn 落盘无库正文、库文件零写入）、regenerate 服务端复用（同一运行身份与原 revision）与索引未命中的显式失败。
- Fix the B3a review gap: regenerate no longer silently falls back to the legacy background when the original InteractiveRun index is missing (process restart or expired index) — a library-bound turn would previously regenerate with legacy lore without any error. The regenerate path now blocks explicitly when the run index misses or is unavailable: the freshly created fallback run is rolled back, the model never starts, and a stable `stale` error code with fixed sanitized copy reports that the original run background is unavailable; the registry-level "miss creates a bare run" semantics are unchanged, and the block lives only in the regenerate-reuse guarantee. Also adds an end-to-end integration test for the library turn through `startInteractiveTask` (real library file and real interactive store with a fake streaming model endpoint): it covers the final model input (library-mode system prompt and `read_library_item` tool mount, byte-identical ephemeral library prefix, ungranted items absent), persistence isolation (turn persisted without library content, library file byte-identical), server-side regenerate reuse (same run identity and original revision), and the explicit failure on an index miss.

- 完成 Library 游戏后端接线（B3a/L3.2）：互动故事任务按 `background_source`（legacy|library|none）三态裁定背景——新回合携带 `library_context` 时按服务端派生 scopeKey 绑定库（bind-before-start），系统提示经模式感知的单源 composition 构建（同一对象作 runner instruction 与 `SystemPromptLog` 计费锚点），初始装配正文只经 ephemeral 输入逐字节前置，`read_library_item` 替换旧 lore 工具挂载；regenerate/reconnect 不读请求——从服务端 InteractiveRun 复用原背景模式与库绑定（固定原 revision 重绑，漂移显式 `revision_conflict`），客户端重发不同绑定或对 bare 运行发库背景/对库运行发显式 none 显式 `invalid_request`（显式 legacy 声明归一内部空串，旧客户端兼容）。library/显式 none 模式下旧 Lore 三通道全关（系统提示替换表、稳定上下文与常驻资料排除、lore 工具不挂载），conversation 与导演侧同步门控；`library_context_state` 脱敏状态事件与脱敏错误事件（固定文案不泄露内部细节）按互斥规则下发；释放路径幂等（含取消与启动失败）。未声明/legacy 请求逐字节保持旧行为；新增 prompts/agent/app 三层 18 项测试。
- Wire the Library into the game backend (B3a/L3.2): interactive story tasks now resolve their background through `background_source` (legacy|library|none) — a new turn carrying `library_context` binds the library with a server-derived scopeKey before start (bind-before-start), the system prompt is built once through a mode-aware single-source composition (the same object serves as the runner instruction and the `SystemPromptLog` billing anchor), the initial assembly body is delivered only as an ephemeral input prepended byte-identically, and `read_library_item` replaces the legacy lore tools. Regenerate/reconnect never trusts the request — the background mode and library binding are reused from the server-side InteractiveRun (rebinding pinned to the original revision; drift fails explicitly with `revision_conflict`), while a client resending a different binding, sending a library background for a bare run, or an explicit none for a library run fails explicitly with `invalid_request` (an explicit legacy declaration is normalized to the internal empty string for old-client compatibility). In library/explicit-none modes all three legacy lore channels are closed (system-prompt replacement table, stable-context and resident-lore exclusion, no lore tools mounted), with matching gates in the conversation and director paths; the sanitized `library_context_state` event and a sanitized error event (fixed copy, no internal detail leaks) are emitted under mutual exclusion; the release path is idempotent (including cancel and failed starts). Undeclared/legacy requests keep byte-identical legacy behavior; 18 new tests across the prompts/agent/app layers.

- 完成 Library 写作前端接线（B2b/L3.1）：库工作区加载预览新增「带入写作」入口（未保存草稿或写作侧无打开的书时禁用），点击后写入进程内一次性交接——仅携带已保存库的 `libraryId`+`expectedRevision`+`manualItemIds` 与脱敏展示摘要，L2 预览的 `autoItemIds` 不进入交接——并切回写作模式；写作端挂载后仍可消费新交接，消费时与既有世界背景互斥单选（后带入者获胜，另一侧恢复无背景原状态）。发送体按冻结契约显式声明 `background_source=library` 并携带 `library_context`（camelCase），库模式下剥离旧 `lore_references`（与库背景同现即 400），且永不携带 consumer/scopeKey/runContextId 等服务端派生身份；SSE `library_context_state` 状态条区分 bound/active（库无 degraded：绑定期失败直接阻断启动），清除后恢复 none 原状态。定向测试覆盖挂载后交接、双向互斥、引用剥离与状态条；i18n 中英成对。
- Wire the Library into the writing frontend (B2b/L3.1): the load-preview tab in the library workspace gains a "bring into writing" entry (disabled for unsaved drafts or when no book is open on the writing side); clicking it writes an in-process one-shot handoff — carrying only the saved library's `libraryId`+`expectedRevision`+`manualItemIds` plus a sanitized display summary, never the preview-only `autoItemIds` — and switches back to writing mode. The writing side can still consume a handoff created after it mounted, and consumption is mutually exclusive with the world background (the latest explicit choice wins; the other side is restored to its no-background state). The request body declares `background_source=library` with a camelCase `library_context` per the frozen contract, strips legacy `lore_references` in library mode (overlap would be a 400), and never carries consumer/scopeKey/runContextId or any other server-derived identity; a `library_context_state` status bar distinguishes bound/active (no degraded for library: bind failures block the start) and clearing restores the bare none state. Targeted tests cover post-mount handoffs, exclusion in both directions, reference stripping, and the status bar; i18n strings ship in Chinese and English pairs.
- 修正 B2a 审查缺口（NEED REVISION 轮）：其一，新库模式不再叠加旧 Lore——区分“未声明背景来源”与“显式 `background_source=none`”（传输层新增 `BackgroundSourceExplicit`，推断的 none 不等于显式 none），library 与显式 none 模式下旧 Lore 三条注入通道全关：系统提示的 StateContext/审计组成排除 lore 片段、稳定上下文取排除 lore 的变体（`lore_references` 在传输层与 library/显式 none 同现即 400 `background_source_conflict`，app 层防御性再拒），显式 none 模式额外不挂任何背景读取工具，未声明/legacy 请求逐字节保持旧行为；其二，计费与审计改用实际送模的系统提示——app 层以单一来源 `BuildBackgroundInstructionComposition` 按模式构建一次 composition，同一对象既作 runner 构建输入又作 `RunOptions.SystemPromptLog`（计费点 `ChargeExternal` 与提示审计不再使用默认提示文本）。新增测试直接断言最终模型消息无旧 Lore、显式 none 无背景、未声明请求保持兼容、计费文本与库模式提示一致；提示词替换表扩为 legacy/library/none 三列，双端守护测试同步。
- Close the two B2a review gaps (NEED REVISION round): first, library mode no longer overlays legacy lore — the transport now distinguishes an undeclared background source from an explicit `background_source=none` via a new `BackgroundSourceExplicit` flag (an inferred none is not an explicit none), and in library or explicit-none mode every legacy lore injection channel is closed: the system-prompt StateContext and audit composition exclude the lore fragment, the stable context uses the lore-excluding variant, `lore_references` overlapping library/explicit-none is rejected as 400 `background_source_conflict` at the transport layer (with a defensive re-check in the app layer), and explicit-none mode additionally mounts no background read tools at all, while undeclared/legacy requests keep byte-identical legacy behavior; second, billing and auditing now use the system prompt actually sent to the model — the app builds one mode-aware composition through `BuildBackgroundInstructionComposition` and feeds the very same object to both the runner construction and `RunOptions.SystemPromptLog`, so the `ChargeExternal` billing point and prompt audit no longer measure the default prompt text. New tests assert directly that the final model messages carry no legacy lore, explicit-none has no background, undeclared requests stay compatible, and the billed text equals the library-mode prompt; the prompt replacement table is widened to legacy/library/none triple columns with the guard tests updated on both ends.
- 完成 Library 写作后端接线（B2a/L3.1）：传输层冻结 `background_source`（legacy|library|none）+ `library_context{libraryId,expectedRevision,manualItemIds[]}`，与 world 字段同现 400 `background_source_conflict`，不做优先级吞并；写作任务在启动前按服务端派生 scopeKey 绑定库（bind-before-start），绑定期失败按稳定错误码映射 HTTP（400/403/409/503/413）并阻断启动，系统提示与逐轮历史经 `ChargeExternal` 计入同一累计预算。旧背景通道逐一关闭：library 模式不挂载 lore 工具并挂载持有本次绑定 Run 的 `read_library_item`，系统提示中的 lore 工具指引整段替换为库读取指引（成对替换表，双端测试守护同步）。持久化隔离四通道（Session 上下文不保留库工具结果、工具日志只记字节数、SSE/display/ledger 共用的 tool_result 事件在唯一脱敏点只携带条目元数据、model-input 日志过滤库背景头部），新增 `library_context_state` 脱敏状态事件与 run ledger 的 `library_id`/`library_revision` 增记。受控 wire 契约回写 L3 计划 §8.8，供 B2b 前端与 B2c 闭环对接；interactive 显式拒绝 library 背景直至 B2b。
- Wire the Library into the writing backend (B2a/L3.1): the transport freezes `background_source` (legacy|library|none) plus `library_context{libraryId,expectedRevision,manualItemIds[]}`, rejecting any overlap with world fields as 400 `background_source_conflict` without priority swallowing; a writing task binds the library before start with a server-derived scopeKey (bind-before-start), bind failures map to HTTP via stable error codes (400/403/409/503/413) and block the start, and the system prompt plus per-turn history are charged into the same cumulative budget through `ChargeExternal`. Every legacy background channel is closed for library mode: the lore tools are not mounted, a `read_library_item` tool holding the bound run is added instead, and the lore tool guidance inside the system prompt is replaced wholesale with library-read guidance via a paired replacement table guarded by tests on both ends. Persistence isolation is closed across four channels (session context never retains library tool results, tool logs record byte counts only, the tool_result event shared by SSE/display/ledger carries sanitized item metadata at the single redaction point, and the model-input log filters the library background header), plus a sanitized `library_context_state` event and `library_id`/`library_revision` metadata in the run ledger. The controlled wire contract is written back to L3 plan §8.8 for the B2b frontend and the B2c loop; interactive mode explicitly rejects library backgrounds until B2b.
- 修复 Library 运行时授权核心（B1）的三处预算口径缺陷：初始装配计费改为覆盖完整交付文本（冻结抬头+ModelView JSON，与 `EstimatedTokens()` 同口径）；预留输出零值 Config 归一为默认（与同结构体其他字段一致），显式越界（≥ 累计 token 上限）在绑定期显式 `invalid_request`，不再静默改写成误导性阻断；新增 `ChargeExternal` 受控通道，把运行期已知成本（逐轮历史增长、真实系统提示、按需读取工具结果外包装）计入同一累计计数器。三条失败回归测试先行，全包 17 用例与 app 定向用例通过。
- Fix three budget-accounting defects in the Library runtime authorization core (B1): the initial assembly now charges the full delivered text (frozen header + ModelView JSON, matching `EstimatedTokens()`); a zero-value Config normalizes the reserved output to the default like its sibling fields, while an explicit out-of-range reserve (>= the cumulative token budget) is rejected as `invalid_request` at bind time instead of a silent rewrite into a misleading block; and a controlled `ChargeExternal` channel lets the wiring layer charge runtime-known costs (per-turn history growth, the real system prompt, on-demand tool envelopes) into the single cumulative counter. Each fix is covered by a failing-first regression test; all 17 package tests and the targeted app tests pass.
...
- L1.2 wires the standalone work library HTTP and frontend create/list/edit/save flow while keeping the existing public materials accessible. Adds load retry, draft retention on failed creation, conflict feedback, and frontend tests. Load modes are stored and displayed only; model access is not enabled in this phase.
- L1.1 冻结独立作品设定库的最小数据契约并落地 `internal/library` 领域与存储：库落在全局数据目录 `libraries/library-<id>.json`，独立于书籍与 World；服务端生成库 ID 与 schemaVersion，库级并发用文件内容哈希 CAS（`expected_revision`）、条目级并发用条目 `baseUpdatedAt`，全部写入都经 `revisionfile` 路径锁内读改写加原子替换，崩溃不留半个文件。条目复用旧 Lore 词表（类型/重要度/ID 词干直接复用 `internal/book` 实现，档位复用 `book` 常量并刻意去掉 major 自动常驻），新增 event/ability 类型；允许同名条目、引用一律走稳定 ID，ID 分配后改名不改 ID；更新省略字段保持原值，正文用指针区分“未提交”与“显式清空”。来源只保存身份与版本指针（master/lore/world/file/manual），拒绝本机绝对路径；只读引用条目（origin=reference）不得改写正文，显式转本地改编才可写并保留来源。关系两端必须是库内稳定 ID，拒绝自指与重复，悬空引用是硬错误；事件是一种条目、时间线是它的派生视图（排序稳定可复现，删除事件不影响参与人物）；删除条目先返回影响明细，未级联即拒绝并保持数据不变，级联时同步清理关系与事件参与者引用。新增契约文档 `docs/plans/LIBRARY_L1_DATA_CONTRACT.md` 与 19 项契约测试。本步只做资料本身：不接写作/游戏/叙界/沙盒取材、不调用模型、不迁移旧数据、不新建第二套原件或翻译体系。
- L1.1 freezes the minimal data contract for the standalone work-settings library and lands the `internal/library` domain and storage: a library lives in the global data directory as `libraries/library-<id>.json`, independent of any book or World. The server generates the library ID and schemaVersion; library-level concurrency uses a content-hash CAS (`expected_revision`) and item-level concurrency uses the item `baseUpdatedAt`; every write goes through `revisionfile` as a locked read-modify-write plus atomic replacement, so a crash never leaves half a file. Items reuse the existing Lore vocabulary (item types, importance and ID stems delegate to `internal/book`; load modes reuse `book` constants and deliberately drop the major-to-resident promotion) and add the event/ability types. Duplicate names are allowed and every reference uses the stable ID, which never changes on rename; omitted fields keep their stored value, and the body uses a pointer to distinguish "not submitted" from "explicitly cleared". Sources store only an identity and version pointer (master/lore/world/file/manual) and reject host absolute paths; a read-only reference item (origin=reference) cannot have its body rewritten, while an explicit switch to a local adaptation allows edits and keeps the source pointer. Both relation ends must be stable in-library IDs; self-relations and duplicates are rejected and dangling references are a hard error. An event is an item and the timeline is its derived view (stable, reproducible ordering; deleting an event never touches its participants). Deleting an item first returns its impact and is refused without an explicit cascade, which then clears the referencing relations and event participant links. Adds the contract document `docs/plans/LIBRARY_L1_DATA_CONTRACT.md` and 19 contract tests. This step covers the material itself only: no writing/game/narraverse/sandbox consumption, no model calls, no legacy data migration, and no second source-material or translation system.
...
- Phase 3.2-C/D 完成 Narraverse 与 Module4 的只读 World Context 正式接入：Denova 顶层固定在 `127.0.0.1`，iframe 以同端口 `localhost` 形成跨源边界，并通过一次性启动 fragment、HttpOnly SameSite=Strict 宿主会话和受控 bind/call/unbind 路由代理共享模型调用。World Console 新增两种显式交接；宿主固定 consumer、严格过滤 iframe 消息及模型请求，不向 iframe 下发 World Ref、runContext、ModelView 或凭据。加入旧浏览器存储的哈希迁移、IPv6 localhost 回环伴随代理，以及“首个模型请求必须等待绑定完成”的竞态修复。正式 Edge 隔离验收 10/10 通过，Narraverse/Module4 都确认携带只读背景，World 文件哈希前后不变。
- Phase 3.2-C/D completes the production read-only World Context integration for Narraverse and Module4. The Denova top-level host remains on `127.0.0.1`, the iframe uses same-port `localhost` as a cross-origin boundary, and shared model calls are proxied through a one-shot launch fragment, an HttpOnly SameSite=Strict host session, and controlled bind/call/unbind routes. The World Console adds explicit handoffs for both consumers; the host fixes the consumer and strictly filters iframe messages and model requests without exposing World Refs, run contexts, ModelViews, or credentials. It adds hash-verified legacy browser-storage migration, an IPv6 localhost loopback companion, and a race fix that makes the first model request wait for binding. Isolated production Edge acceptance passed 10/10, both Narraverse and Module4 received read-only context, and the World file hash stayed unchanged.
- Phase 3.2-C0 冻结 iframe 安全边界：Denova 顶层宿主固定为 `127.0.0.1`，Narraverse/Module4 iframe 使用同端口 `localhost` 形成真实跨源隔离；一次性 fragment secret 只用于换取 HttpOnly SameSite=Strict 宿主会话，iframe 永不持有 Cookie、World Ref、runContext 或 ModelView。新增隔离 Edge 证明与迁移契约测试，验证来源/存储隔离、secret 单次消费、frame 独立撤销、进程重启失效，以及 localStorage/IndexedDB 跨源迁移的哈希一致、源保留、冲突不覆盖和失败不可加载。本步不注册生产 API、不改 Module3/4、不调用模型。
- Phase 3.2-C0 freezes the iframe security boundary: the Denova top-level host stays on `127.0.0.1`, while Narraverse/Module4 iframes use `localhost` on the same port to establish real cross-origin isolation. A one-shot fragment secret only exchanges for an HttpOnly SameSite=Strict host session; the iframe never receives the cookie, World Ref, run context, or ModelView. Isolated Edge proof and migration-contract tests verify origin/storage isolation, one-shot secret use, per-frame revocation, restart invalidation, and hash-consistent cross-origin localStorage/IndexedDB migration with source retention, conflict protection, and non-loadable failure state. This step registers no production API, changes no Module3/4 code, and invokes no model.
- Phase 3.2-B 补齐游戏侧 World Context 闭环：World Console 的游戏入口统一经过 launchGame；interactive context-analysis 接受只读 World Ref 并签发/复用一次性 analysis handle；首次游戏请求完成 pending context 的 claim/move/consume，阻断错误通过脱敏 SSE error 事件返回而不静默降级 bare；regenerate 由服务端按 TurnIndex 复用 InteractiveRun，切故事/分支清理旧交接状态，前端展示 active/degraded/none 状态。新增后端与 StoryStage/WorldConsole 回归测试，未改 World 真源、Module3/4、iframe/capability 或模型代理。
- Phase 3.2-B completes the game-side World Context loop: the World Console game entry consistently uses launchGame; interactive context analysis accepts a read-only World Ref and issues or reuses a one-shot analysis handle; the first game request claims/moves/consumes pending context, while blocking errors return through a redacted SSE error event instead of silently degrading to bare mode; regenerate reuses InteractiveRun by server-side TurnIndex, stale handoffs are cleared on story/branch changes, and the frontend exposes active/degraded/none status. Backend, StoryStage, and WorldConsole regression tests were added; World truth, Module3/4, iframe/capability, and the model proxy were not changed.
- Phase 3.2-A6 闭合写作世界背景的客户端可用链路：写作发送体在直接发送时携带冻结 `world_context`（Ref），先做上下文分析后的首次发送只携带一次性 `analysis_handle`、后续新 run 回到 Ref；SSE 在模型内容前恰好下发一次 `world_context_state`（none/active/degraded，bound 为纯客户端待发送态），AgentPanel 新增只读状态条区分 bound/active/degraded 与单列 analysis_handle_status（绝不把失效 handle 伪装成 degraded），并可一键清除「后续新 run」背景（不影响当前运行、不发 World PUT）。新增纯内存 `WorldContextRunProvider` 展示桥与 `world-context-wire`（请求体构造、状态白名单解析，未知 state 安全降级）；LaunchProvider 提升到 main 根层以覆盖 App 自身的 useAgentChat。reconnect 走 GET stream 无 body，天然不重交 Ref/handle；请求失败保留 Ref、成功启动后 handle 立即清空；不写浏览器存储、不进 Zustand、不展示 runContextId/scopeKey/fingerprint 等内部字段，游戏/Narraverse/Module4 不变。
- Phase 3.2-A6 closes the client-side writing world-context loop: a direct send carries the frozen `world_context` Ref, while the first send after a context analysis carries only the one-shot `analysis_handle` and later runs fall back to the Ref. The SSE emits exactly one `world_context_state` before model content (none/active/degraded; bound is a client-only pending state). AgentPanel gains a read-only state bar that separates bound/active/degraded from the standalone analysis_handle_status (an unusable handle is never presented as degraded) and a one-click control that clears the background for future runs without affecting the current run or sending a World PUT. It adds the in-memory-only `WorldContextRunProvider` display bridge and `world-context-wire` (request-body builder and whitelist status parsing; unknown states degrade safely), and lifts the LaunchProvider to the main root so it covers App's own useAgentChat. Reconnect uses a bodiless GET stream and never resends Ref/handle; a failed request keeps the Ref and a started request clears the handle immediately. No browser storage or Zustand, no runContextId/scopeKey/fingerprint shown; game/Narraverse/Module4 are unchanged.
- Phase 3.2-A5 在 World Console「世界上下文」分区新增显式「带入写作」交接：新增纯内存 `WorldContextLaunchProvider`（React Context，唯一持有一个待消费写作 Ref；不写 localStorage/sessionStorage/IndexedDB、不进 Zustand，刷新即空），挂在 App 根部同时覆盖 ModeRouter 与写作聊天；控制台在 dirty/超限/无已保存 revision/无主书时硬阻断，通过单次离开确认并成功切换主书后才把已保存 World 的 Ref（worldId+expectedWorldRevision+Selection 与脱敏展示摘要）写入 Provider 并切到写作模式，切书失败或取消都留在原页、不写 Ref、不切模式、不发 PUT。本轮只建立交接桥与入口，不在写作端发送 Ref、不调用模型、不创建 runContext/analysisHandle，游戏/Narraverse/Module4 不变。
- Phase 3.2-A5 adds the explicit "bring into writing" handoff to the World Console context section: a new in-memory-only `WorldContextLaunchProvider` (React Context holding at most one pending writing Ref; no localStorage/sessionStorage/IndexedDB, no Zustand, cleared on refresh) is mounted at the App root so it covers both ModeRouter and the writing chat. The console hard-blocks the handoff when dirty, over selection limits, without a saved revision, or without a primary book; after the single leave confirmation and a successful primary-book switch it writes the saved World Ref (worldId+expectedWorldRevision+Selection plus a sanitized display summary) into the provider and switches to writing, while a failed/cancelled book switch stays on the page with no Ref, no mode change, and no PUT. This round only builds the handoff bridge and entry point: it does not yet send the Ref from writing, calls no model, creates no runContext/analysisHandle, and leaves game/Narraverse/Module4 unchanged.
- Phase 3.2-A4 闭合写作 context-analysis → analysisHandle → 首次 chat 交接：context-analysis 提交有效 Ref 时由服务端按当前工作区+活跃会话派生 sessionKey，经既有生命周期核心建立/幂等复用 pending runContext 并签发短期 `analysis_handle`/`analysis_handle_expires_at`（重复分析返回同一 handle 与同一 pending runContext、不延长 TTL、不启动模型或 Task）；分析展示直接读取该 pending runContext 的最终 ModelView bytes，首次 chat 经 claim+`MoveScopeByID` 迁移到 `task:<id>` scope 后 consume，迁移不重投影、不换 salt、不复制 bytes，分析与送模逐字节同源。claim 新增服务端会话归属校验，跨会话/跨工作区即便持有 token 也判 `ignored_invalid`；move 成功后采用 consume 的真实返回状态（并发取消/过期不再被假定为 consumed），runContext 已在 task scope 时仍由任务恰好释放一次。普通无 Ref 分析不创建 handle；可降级运行时错误返回普通分析并标 `degraded`（不签发 handle），阻断错误直接返回。响应只暴露稳定状态与脱敏摘要，不下发 runContextId/scopeKey/fingerprint/sourceRef/runSalt。
- Phase 3.2-A4 closes the writing context-analysis to analysisHandle to first-chat handoff: when context-analysis carries a valid Ref, the server derives sessionKey from the current workspace and active session and, through the existing lifecycle core, creates or idempotently reuses a pending runContext and issues a short-lived analysis_handle/analysis_handle_expires_at (repeated analysis returns the same handle and pending runContext without extending the TTL, and starts no model or Task). The analysis view reads that pending runContext final ModelView bytes directly, and the first chat claims it, migrates it to the task scope via MoveScopeByID, then consumes it with no reprojection, no salt change, and no byte copy, so analysis and model input are byte-identical. claim now verifies server-side session ownership: a token from another session/workspace is ignored_invalid even when present. After a successful move the real consume status is honored (concurrent cancel/expiry is no longer assumed consumed), and a runContext already in the task scope is still released exactly once by the task. Plain Ref-less analysis creates no handle; degradable runtime errors return normal analysis marked degraded (no handle issued), while blocking errors return directly. Responses expose only stable status and sanitized summary, never runContextId/scopeKey/fingerprint/sourceRef/runSalt.
- Phase 3.2-A3 把绑定 runContext 的最终 ModelView bytes 作为“临时只读世界背景”注入真实写作模型输入：新增 `agent.EphemeralWorldContextInput`（冻结三行英文抬头 + 逐字节 ModelView JSON，前置为 leading user message）与 `agent.ModelInputMessages` 装配缝；`Runtime.Run` 只把前置后的消息副本送入 `runner.Run`，`PrepareMessages` 提交进 Session 的持久 history、context/run ledger、display、export 与压缩 source/summary 均不包含世界正文。压缩预算通过 `ContextCompactionInput.ReservedEphemeralWorldTokens` 计入临时输入占用，但世界背景绝不进入压缩来源；日志只记录状态、字节数、token 估算与消息计数，不记录正文/ID/sourceRef/runSalt/handle。bare（无 Ref/handle）传零值，模型输入与基线逐结构一致。context-analysis 复用同一临时输入构造函数展示本次独立物化结果；跨请求逐字节复用由 A4 pending runContext/analysisHandle 闭环承接（本轮不创建 handle）；新增无 Registry 的只读装配 `WorldContextService.BuildWritingEphemeralWorld` 与 `worldcontext.MaterializeModelViewBytes`（不创建 runContext、不写 World）。
- Phase 3.2-A3 injects the bound runContext's final ModelView bytes into the real writing model input as an ephemeral read-only world background: new `agent.EphemeralWorldContextInput` (frozen three-line English header plus byte-exact ModelView JSON, prepended as a leading user message) and the `agent.ModelInputMessages` assembly seam. `Runtime.Run` only sends the prepended message copy to `runner.Run`; the persisted history committed by `PrepareMessages`, context/run ledgers, display, export, and compaction source/summary never contain world text. The compaction budget accounts for the ephemeral input via `ContextCompactionInput.ReservedEphemeralWorldTokens`, but world background never enters the compaction source; logs record only status, byte count, estimated tokens and message counts, never body/IDs/sourceRef/runSalt/handle. A bare request (no Ref/handle) passes the zero value and stays structurally identical to baseline. context-analysis reuses the same ephemeral-input constructor for its independently materialized result; byte-identical reuse across requests is deferred to the A4 pending runContext/analysisHandle handoff (no handle is created this round); adds the registry-free read-only assembler `WorldContextService.BuildWritingEphemeralWorld` and `worldcontext.MaterializeModelViewBytes` (no runContext created, World never written).
- Phase 3.2-A2 把写作任务改为 bind-before-start：先 `newPendingTask` 仅分配 Task ID（不起模型 goroutine），再经 `WorldContextService.resolveWritingRun` 完成 Ref/handle 裁定，最后设置 activeTask 并 `task.start`，模型 goroutine 不可能早于绑定。direct Ref 的 consumer 固定为 writing、scopeKey 由服务端 `task:<id>` 派生、使用已保存 World revision；handle 仅从进程内注册表 claim，fingerprint 一致时用 `MoveScopeByID` 把 pending runContext 迁移到 task scope 再 consume（不复制 ModelView bytes），不一致时回滚 handle、按 Ref 新建并记 `ignored_conflict`（新增该状态），失效/过期/已消费 handle 不阻断写作且绝不用于恢复背景；阻断错误（selection_invalid/revision_conflict/world_archived/context_ref_mismatch/consumer_not_trusted/invalid_request）废弃 pending Task 并返回，`world_not_found`/`world_unavailable`/`context_unavailable`/`projection_failed` 与仅 consumer_window 过小的预算错误降级为 bare，其余预算错误阻断。任务结束（成功/失败/取消）对 task scope 运行引用只释放一次，bare 请求 Registry 零增量，World 只读。会话新建/切换/删除统一经 `abortActiveTaskLocked` 让旧写作会话未结算 handle 与其 pending context 失效。
- Phase 3.2-A2 converts writing tasks to bind-before-start: `newPendingTask` only allocates a Task ID (no model goroutine), then `WorldContextService.resolveWritingRun` settles Ref/handle, and only afterward is activeTask set and `task.start` called, so the model goroutine can never precede binding. A direct Ref fixes consumer to writing, derives scopeKey server-side as `task:<id>`, and uses the saved World revision; a handle is claimed only from the in-process registry: on fingerprint match `MoveScopeByID` migrates the pending runContext to the task scope before consume (no ModelView byte copy), while a mismatch rolls the handle back, binds fresh from the Ref, and records the new `ignored_conflict` status; invalid/expired/consumed handles never block writing and never restore background. Blocking errors (selection_invalid/revision_conflict/world_archived/context_ref_mismatch/consumer_not_trusted/invalid_request) discard the pending Task and return, while `world_not_found`/`world_unavailable`/`context_unavailable`/`projection_failed` and only consumer-window-too-small budget errors degrade to bare; all other budget errors block. The task-scope run reference is released exactly once on success/failure/cancel, a bare request adds zero registry entries, and the World stays read-only. Session create/switch/delete now uniformly invalidates the previous writing session's unsettled handles and pending contexts via `abortActiveTaskLocked`.
- Phase 3.2-A1b 把写作请求解析边界正式接入 `/api/chat` 与 `/api/chat/context-analysis`：handler 对请求体只读一次，同一字节流经 `DecodeWorldContextTransport`（PolicyChat / PolicyContextAnalysis）与 `DecodeChatRequestWithoutWorldContext` 双重解码，拒绝尾随/多个 JSON；World 控制字段经新增的显式 app 输入类型 `WritingTaskInput{Request, World}`（`internal/app/world_context_writing.go`，含 `WritingWorldControl`）继续传递，绝不进入 `agent.ChatRequest`。chat 允许 Ref/handle 同现；context-analysis 允许 Ref、非空 handle 返回 400；传输层领域错误回稳定 code 与脱敏文案，旧 bare 请求行为逐结构不变。本步只建立解析与输入契约，不做 bind-before-start、不启动模型、不创建 runContext/handle。
- Phase 3.2-A1b wires the writing request-parsing boundary into `/api/chat` and `/api/chat/context-analysis`: the handler reads the body exactly once and runs the same bytes through `DecodeWorldContextTransport` (PolicyChat / PolicyContextAnalysis) and `DecodeChatRequestWithoutWorldContext`, rejecting trailing/multiple JSON. World control fields travel through a new explicit app input `WritingTaskInput{Request, World}` (`internal/app/world_context_writing.go`, with `WritingWorldControl`) and never enter `agent.ChatRequest`. Chat allows Ref and handle together; context-analysis allows Ref and rejects a non-empty handle with 400; transport domain errors return stable sanitized codes, and legacy bare requests stay structurally identical. This step only establishes parsing and the input contract: no bind-before-start, no model start, and no run context/handle creation.

- Phase 3.2-A1a 新增写作运行传输 DTO 与纯解码契约 `internal/api/handlers/world_context_transport.go`：`DecodeWorldContextTransport` 从请求体中分离可选 `world_context`（严格 camelCase：worldId/expectedWorldRevision/selection）与顶层 `analysis_handle`，selection 复用 `worldcontext.DecodeSelection` 冻结形状校验；顶层对既有 `agent.ChatRequest` 字段全量放行（旧请求逐字段兼容），仅对 world_context 子树严格拒绝未知字段，并在顶层与子树双层拦截 consumer/scope/scopeKey/run_scope/runContextId/run_context_id/interactiveRunId/capability/snapshot/modelView 越权字段，拒绝尾随/多 JSON/非单一对象。区分 PolicyChat（允许 Ref 与 handle 同现、不互斥）与 PolicyContextAnalysis（只允许 Ref，非空 handle 拒绝）；handle 必须是 32~128 位 base64url 字符串，空串/null 等同未携带。`DecodeChatRequestWithoutWorldContext` 保证控制字段不进入 `agent.ChatRequest`。本步不接 handler、不启动模型、不创建 runContext/analysisHandle。
- Phase 3.2-A1a adds the writing runtime transport DTO and pure decode contract `internal/api/handlers/world_context_transport.go`: `DecodeWorldContextTransport` separates the optional `world_context` (strict camelCase: worldId/expectedWorldRevision/selection) and top-level `analysis_handle` from a request body, reusing `worldcontext.DecodeSelection` for frozen selection-shape validation. Top-level existing `agent.ChatRequest` fields are all allowed (field-by-field backward compatibility); only the world_context subtree strictly rejects unknown fields, while both the top level and subtree reject the privilege fields consumer/scope/scopeKey/run_scope/runContextId/run_context_id/interactiveRunId/capability/snapshot/modelView, as well as trailing/multiple/non-single-JSON bodies. It distinguishes PolicyChat (Ref and handle may coexist) from PolicyContextAnalysis (Ref only; a non-empty handle is rejected). A handle must be a 32-128 char base64url string; empty string/null mean absent. `DecodeChatRequestWithoutWorldContext` ensures control fields never enter `agent.ChatRequest`. This step wires no handler, starts no model, and creates no run context/analysis handle.
- Phase 3.1C3b 将 Timeline 八态兼容规则接入真实编辑界面：旧 `canon` 与未知非空分类显示明确旧数据标签，空字符串、`null` 与缺失字段按普通背景展示；界面挂载和只读查看不改写原始值，只有用户显式选择时才升级为 `background`、`historical` 或 `planned`。有条目与空状态均显示“世界背景、非跨模式剧情真相”的边界说明。
- Phase 3.1C3b wires the eight-state Timeline compatibility rules into the editor UI: legacy `canon` and unknown non-empty categories show explicit legacy labels, while empty strings, `null`, and missing fields display as ordinary background. Mounting or read-only viewing never rewrites raw values; only an explicit user selection upgrades a value to `background`, `historical`, or `planned`. Both populated and empty states explain that Timeline is world background, not shared story truth across modes.
- Phase 3.1C3a 补齐 Timeline 八态兼容展示语义：`background`、`historical`、`planned` 保持当前三值，旧 `canon` 只归一为历史展示并标为“历史（旧数据）”，未知非空值归一为背景并标为“背景（旧数据）”，空字符串、`null` 与缺失字段按普通背景展示；归一化仅用于展示，不改写原始 World 值，新建和显式编辑仍只允许三种当前值。补充 Timeline 说明，明确它是世界背景，不是跨模式共享的剧情真相。
- Phase 3.1C3a completes the eight-state Timeline compatibility semantics: `background`, `historical`, and `planned` remain the only current values; legacy `canon` is normalized only for display and labeled "Historical (legacy data)"; unknown non-empty values are normalized to background and labeled "Background (legacy data)"; empty strings, `null`, and missing fields display as ordinary background. Normalization never rewrites the raw World value, and new or explicitly edited entries accept only the three current values. Timeline help text clarifies that it is world background, not shared story truth across modes.
...
- Phase 3.1C1 second-pass review tightened the TypeScript read-only boundary of derived binding results: reference groups and binding views no longer expose mutable types, preventing derived results from becoming a side channel for World mutation. No runtime storage or API was added.
- Phase 3.1C1 新增绑定可观测性纯派生层 `binding-observability.ts`：`bindingUsages` 稳定扫描角色/地点/势力对某绑定的全部引用（多对多、非单一 owner、不新增 ownerId/references 反向索引）；`bindingRemovalImpact` 只计算不移除，复用 `bindingScopeOf`，列出将解除引用的实体并判定 orphan 候选（world scope 零引用仍保留、entity scope 零引用才是候选）；`bindingStoredStatus` 仅依据持久化的 masterRevision 区分 unchecked/baseline，刻意不产出联网健康态。纯函数不改 World、不读 Master、不联网、不持久化。
- Phase 3.1C1 adds the pure derived binding-observability layer `binding-observability.ts`: `bindingUsages` stably scans all character/location/faction references to a binding (many-to-many, no single owner, no ownerId/references reverse index); `bindingRemovalImpact` computes without mutating, reuses `bindingScopeOf`, lists entities that would be detached and flags orphan candidates (world scope survives with zero references; only entity scope with zero references is a candidate); `bindingStoredStatus` derives unchecked/baseline solely from the persisted masterRevision and deliberately emits no online health state. The pure functions never modify the World, read no master data, and perform no network or persistence.
- Phase 3.1B 将世界上下文预览正式接入 World Console（第八个“世界上下文”分区）：新增受控选择控件（基调/规则按索引/角色/地点/势力/时间线/仅 world-scope 资料，consumer 仅 writing/game，前端按后端冻结上限预检且超限只提示不静默截断）；ready/stale 分段展示 Identity/Setting/Characters/Locations/Factions/Timeline/Materials、canonicalSelection、统计、短指纹、可读 omissions/warnings 与 sourceTable 来源跳转（未知 kind/code 安全降级）；选择与预览只活在当前控制台会话，打开/切分区零自动请求，点“生成预览”恰好一次，草稿 dirty、保存成功或 CAS 重载后旧预览标 stale，切世界清空，来源跳转只切分区、不改 World，不创建 runContext、不调模型、不访问浏览器持久化、不改 Module3/4。
- Phase 3.1B integrates the World Context Preview into the World Console as its eighth ("World Context") section: a controlled selection control (tone, rules by index, characters/locations/factions/timeline, and world-scope-only materials; consumer is writing/game only; the client prechecks the frozen server limits and warns without silently truncating); ready/stale segmented views for Identity/Setting/Characters/Locations/Factions/Timeline/Materials, canonical selection, stats, a short fingerprint, human-readable omissions/warnings, and source-table jumps (unknown kinds/codes degrade safely). Selection and preview live only for the current console session: no auto request on open or section switch, exactly one request per Generate click, stale after dirty/save/CAS-reload, cleared on world switch, source jumps only change section without touching the World. No run context, no model call, no browser persistence, and no Module3/4 changes.

- Phase 3.1A2 新增世界上下文预览前端消费层：`world-context.ts` 作为唯一 camelCase wire 类型来源，`previewWorldContext` 调用只读 `POST /api/worlds/:id/context-preview`；`useWorldContextPreview` 仅在组件内存内持有 selection/preview 与 idle/loading/ready/stale/error 状态（请求序号丢弃过期响应，不写浏览器存储、不进 Zustand），`WorldContextPreviewResult` 只读展示世界概览、已选内容、闭包 omissions 与 warnings 及空态；本步不接入 WorldConsolePage、不做选择控件与来源跳转（属 3.1B）。
- Phase 3.1A2 adds the World Context Preview frontend consumption layer: `world-context.ts` is the single camelCase wire-type source, `previewWorldContext` calls the read-only `POST /api/worlds/:id/context-preview`, `useWorldContextPreview` keeps selection/preview and idle/loading/ready/stale/error state in component memory only (request-sequence drops stale responses; no browser storage and no Zustand), and `WorldContextPreviewResult` renders world identity, included content, closure omissions, warnings, and empty states. This step does not integrate WorldConsolePage or build selection controls/source navigation (those belong to 3.1B).
- Phase 3.1A1 新增只读世界上下文预览端点 `POST /api/worlds/:id/context-preview`：从已保存 World 与 revision 即时派生 UI Projection，使用显式 camelCase 传输 DTO（不直接序列化内部 Selection），稳定错误码与脱敏，请求体上限 64 KiB；不创建 runContext、不占 Registry、不调用模型或 Master 详情，不返回 ModelView/sourceRef/runSalt 等运行态字段。
- Phase 3.1A1 adds the read-only `POST /api/worlds/:id/context-preview` endpoint: it derives the UI Projection on demand from the saved World and revision through explicit camelCase transport DTOs (the internal Selection is never serialized directly), with stable error codes, sanitized errors, and a 64 KiB body cap. It creates no run context, occupies no registry, calls no model or master-asset detail, and never returns ModelView/sourceRef/runSalt runtime fields.
...
- Returning to Narraverse now notifies the embedded app to pull project updates. Together with the one-click local sync bridge, newly created adventures synchronize settings, chapters, and lore between Narraverse and Denova without migrating legacy adventures.
- Denova 新增与写作、游戏同级的“叙界”模式：同源持久 iframe 在模式切换时不会重载，并支持无书籍直接进入、`?mode=narraverse` 深链接、主题/语言同步与失败重试。
- Denova adds Narraverse as a top-level peer of Writing and Game. Its same-origin persistent iframe survives mode switches and supports book-independent startup, `?mode=narraverse` deep links, theme/locale sync, and recoverable loading errors.
- 叙界静态运行时现在随 Denova 前端与嵌入式发布包分发；新增白名单同步脚本，构建不依赖开发机上的绝对源目录。
- The Narraverse static runtime now ships with the Denova frontend and embedded release bundle. A whitelist-based sync script keeps release builds independent of developer-machine source paths.

### Fixed

- 修复 app 自动化测试时序 flake：`TestAutomationMutationCallbackChecksAgentChapterWrites` 的触发评估走 `automationTriggerCoordinator` 后台 worker，inbox 项可见后 worker 仍会继续写「自动运行启动失败」标记等状态；用例未排空在途写就结束，与 `t.TempDir()` 的 RemoveAll 清理竞态（Windows 间歇报 "The directory is not empty"）。按同文件既有模式在用例结束前 `defer app.Close()`（取消并 `wg.Wait()` 等在途评估收尾，同文件另两个触达同一异步路径的用例本就有此后缀）。修复前 30 连跑 5 次失败，修复后 3×30 连跑 0 失败；未改生产逻辑。
- Fixes a timing flake in an app automation test: `TestAutomationMutationCallbackChecksAgentChapterWrites` evaluates triggers through the background `automationTriggerCoordinator` worker, which keeps writing state (such as the auto-run failure mark) after the inbox item becomes visible; the test returned without draining it, racing `t.TempDir()`'s RemoveAll cleanup and failing intermittently on Windows with "The directory is not empty". The test now drains the worker with a deferred `app.Close()` (cancel plus `wg.Wait()` for the in-flight evaluation), matching the two sibling tests in the same file that already reach this async path. 5 failures in 30 runs before the fix, 0 in three consecutive 30-run batches after; no production behavior change.

- 修复无书籍时作品设定库入口被静默弹回书库：`App.tsx` 的无书籍守卫白名单长期只有 `books/narraverse/worlds`，L1 新增的 `library` 未登记，导致没有打开书籍时点击「总资料库」会先渲染一帧再被换回书库，不报错也不提示，「不必先建书就能整理资料」的 L1 目标在界面上不可达。现把白名单抽为 `WORKSPACE_FREE_MODES` 常量（与 `WorkspaceMode` 同处定义），覆盖 books/worlds/library/skills/agents/automations/narraverse；新增模式若忘记登记会有穷举测试报错。同时修复 web 测试环境的两处假红灯：vitest 默认 5s 超时对本机模块图加载过紧（改为 20s，断言标准不变），以及 jsdom 默认 Origin 为 `localhost` 会命中 `main.tsx` 的 localhost→127.0.0.1 重定向、使整个启动流程被跳过（改为规范回环地址）。全量前端测试由基线 3 文件/4 例失败转为 205 文件/1284 例全通过。
- Fixes the work library entry being silently bounced back to the book library when no book is open: the no-book guard in `App.tsx` only ever whitelisted `books/narraverse/worlds`, and the `library` mode added by L1 was never registered, so with no book open clicking the library entry rendered for a frame and was then swapped back to books with no error and no notice, making the L1 goal ("curate material without creating a book first") unreachable through the UI. The whitelist is now the `WORKSPACE_FREE_MODES` constant defined next to `WorkspaceMode` and covers books/worlds/library/skills/agents/automations/narraverse; an exhaustive test fails if a future mode is left unregistered. Also fixes two false red lights in the web test environment: the vitest 5s default timeout is too tight for module-graph loading on this machine (raised to 20s with assertions unchanged), and jsdom's default `localhost` origin matched the localhost→127.0.0.1 redirect in `main.tsx`, skipping the entire startup path (now the canonical loopback address). The full web suite goes from 3 files / 4 tests failing at baseline to 205 files / 1284 tests passing.
...
- Denova release builds once again track the production `cmd/denova` entrypoint. The root runtime ignore is now anchored as `/denova/` so it no longer hides the source directory.
- 叙界静态资源同步白名单现在包含整个模块四运行时目录，Denova embedded 不会因漏同步而回退到缺少模块四脚本的旧页面。
- The Narraverse asset sync whitelist now includes the complete Module 4 runtime directory, preventing Denova embedded builds from silently shipping a page without the Module 4 scripts.
- Denova 的叙界 iframe 版本号已更新，部署模块四静态资源后会主动绕过旧的嵌入页面缓存。
- The Denova Narraverse iframe version is bumped so deployments containing Module 4 assets bypass stale embedded-page caches.
