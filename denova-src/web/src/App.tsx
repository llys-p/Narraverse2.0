...
import { TranslationQueueControl } from '@/features/translation/TranslationQueueControl'
...
function App() {
  ...
  const [booksReturnMode, setBooksReturnMode] = useState<ContentMode>(() => readContentMode())
  const booksReturnModeRef = useRef<ContentMode>(readContentMode())
  const [narraverseModule4Open, setNarraverseModule4Open] = useState(false)
  ...
  const currentBookName = resolveCurrentBookName(workspace, books, summary, t('workbench.noBook'))
  ...
  const handleSetMode = useCallback((nextMode: WorkspaceMode) => {
    ...
    } else if (nextMode === 'ide' || nextMode === 'interactive' || nextMode === 'narraverse') {
  ...
  const handleOpenModule4 = useCallback(() => {
    setNarraverseModule4Open(true)
    handleSetMode('narraverse')
  }, [handleSetMode])
  const handleCloseModule4 = useCallback(() => {
    setNarraverseModule4Open(false)
  }, [])
  ...
  const handleOpenVersions = useCallback(() => {
    setSettingsOpen(false)
    if (mode !== 'ide' && mode !== 'interactive' && mode !== 'narraverse') {
      setMode(booksReturnModeRef.current)
    }
  ...
  return (
    <NovaMotionProvider intensity={motionIntensity}>
      <ModeRouter
        ...
        notice={notice}
        globalTools={<TranslationQueueControl workspace={workspace} mode={mode} />}
...
function readContentMode(): ContentMode {
  ...
  if (value === 'narraverse') return 'narraverse'
  return 'ide'
}

function writeContentMode(mode: ContentMode) {
