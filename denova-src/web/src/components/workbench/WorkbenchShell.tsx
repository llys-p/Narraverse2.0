...
export function WorkbenchShell({
  ...
  const switchNavigationMode = (nextMode: 'ide' | 'interactive' | 'narraverse') => {
    closeSettingsIfOpen()
    if (versionsVisible) onSetRightPanel(null)
    if (nextMode === 'narraverse') onCloseModule4()
    onSetMode(nextMode)
  }
    ...
    <header className="nova-topbar grid h-10 shrink-0 grid-cols-[minmax(0,1fr)_auto] items-center border-b px-3 text-xs">
      <div className="flex min-w-0 items-center gap-2">
        ...
        <div role="group" className="flex h-7 items-center rounded-[var(--nova-radius)] border border-[var(--nova-border)] bg-[var(--nova-surface-2)] p-0.5" aria-label={t('workbench.modeSwitch')}>
          ...
          <button
            type="button"
            aria-pressed={navigationMode === 'narraverse' && !module4Active}
            onClick={() => switchNavigationMode('narraverse')}
            data-onboarding-anchor="mode-narraverse"
            className={`relative overflow-hidden rounded-[6px] px-2.5 py-0.5 text-[11px] transition-colors ${navigationMode === 'narraverse' && !module4Active ? 'bg-[var(--nova-active)] text-[var(--nova-text)]' : 'text-[var(--nova-text-faint)] hover:text-[var(--nova-text-muted)]'}`}
          >
            {navigationMode === 'narraverse' && !module4Active && <motion.span layoutId="workbench-mode-active" className="absolute inset-0 rounded-[6px] bg-[var(--nova-active)]" transition={novaSpring} />}
            <span className="relative z-10">{t('workbench.mode.narraverseButton')}</span>
          </button>
          <button
            type="button"
            aria-pressed={module4Active}
            onClick={onOpenModule4}
            data-onboarding-anchor="mode-module4"
            className={`relative overflow-hidden rounded-[6px] px-2.5 py-0.5 text-[11px] transition-colors ${module4Active ? 'bg-[var(--nova-active)] text-[var(--nova-text)]' : 'text-[var(--nova-text-faint)] hover:text-[var(--nova-text-muted)]'}`}
          >
            {module4Active && <motion.span layoutId="workbench-mode-active" className="absolute inset-0 rounded-[6px] bg-[var(--nova-active)]" transition={novaSpring} />}
            <span className="relative z-10">{t('workbench.mode.module4Button')}</span>
          </button>
        </div>
        </LayoutGroup>
        <BookSwitcher
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            ...
            <div role="group" className="flex h-8 shrink-0 items-center rounded-[var(--nova-radius)] border border-[var(--nova-border)] bg-[var(--nova-surface-2)] p-0.5" aria-label={t('workbench.modeSwitch')}>
              ...
              <button
                ...
                onClick={() => switchNavigationMode('narraverse')}
                data-onboarding-anchor="mode-narraverse"
                className={`relative min-w-0 overflow-hidden rounded-[6px] px-2 py-1 text-[11px] transition-colors ${navigationMode === 'narraverse' && !module4Active ? 'bg-[var(--nova-active)] text-[var(--nova-text)]' : 'text-[var(--nova-text-faint)] hover:text-[var(--nova-text-muted)]'}`}
              >
                {navigationMode === 'narraverse' && !module4Active && <motion.span layoutId="workbench-mobile-mode-active" className="absolute inset-0 rounded-[6px] bg-[var(--nova-active)]" transition={novaSpring} />}
                <span className="relative z-10">{t('workbench.mode.narraverseButton')}</span>
              </button>
              <button
                type="button"
                ...
                onClick={onOpenModule4}
                data-onboarding-anchor="mode-module4"
                className={`relative min-w-0 overflow-hidden rounded-[6px] px-2 py-1 text-[11px] transition-colors ${module4Active ? 'bg-[var(--nova-active)] text-[var(--nova-text)]' : 'text-[var(--nova-text-faint)] hover:text-[var(--nova-text-muted)]'}`}
              >
                {module4Active && <motion.span layoutId="workbench-mobile-mode-active" className="absolute inset-0 rounded-[6px] bg-[var(--nova-active)]" transition={novaSpring} />}
                <span className="relative z-10">{t('workbench.mode.module4Button')}</span>
              </button>
            </div>
          </LayoutGroup>
