...
vi.mock('@/lib/api', async (importOriginal) => {
  ...
  return {
    ...
    cancelTranslationJob: vi.fn(), retryTranslationJob: vi.fn(),
  }
})
...
describe('TranslationQueueControl', () => {
  ...
  it('writes completed Master jobs back to Master without touching current lore', async () => {
    ...
    vi.mocked(applyMasterTranslation).mockResolvedValue({
      ...
      import: {
        kind: 'lorebook', name: 'Harbor', entry_count: 1, created_ids: ['lore-1'], updated_ids: [], conflict_ids: [], skipped_ids: [], failed: [], item_ids: ['lore-1'],
