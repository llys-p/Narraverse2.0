...
describe('InteractiveLayout polling lifecycle', () => {
  it('collapses the director side panel once for a Narraverse imported project', async () => {
    const onToggleRightPanel = vi.fn()
    vi.mocked(getInteractiveStories).mockResolvedValue({ current_story_id: '', stories: [] })
    vi.mocked(readFile).mockImplementation(async (path: string) => {
      if (path === '.narraverse/import-manifest.json') return { workspace: '/workspace', path, content: JSON.stringify({ version: 1, source: 'narraverse' }), revision: '' }
      throw new Error('not found')
    })

    render(<InteractiveLayout workspace="/workspace" rightPanelVisible onToggleRightPanel={onToggleRightPanel} />)
    await waitFor(() => expect(onToggleRightPanel).toHaveBeenCalledOnce())
  })
