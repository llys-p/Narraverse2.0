...
function LoreSettingPanel({
  ...
  const loreDirectoryActions = (
    <>
      <Button className={iconActionClassName} variant="outline" size="icon" disabled={saving || !workspace} onClick={() => setMaterialImportOpen(true)} aria-label={t('settingPanel.materialImport.open')} title={t('settingPanel.materialImport.open')}>
        <FileUp data-icon="inline-start" />
      </Button>
      <Button className={iconActionClassName} variant="outline" size="icon" disabled={saving || items.length === 0} onClick={() => setLoreBatchTranslationOpen(true)} aria-label={t('settingPanel.translationBatch.open')} title={t('settingPanel.translationBatch.open')}>
  ...
  return (
    <section className="h-full min-h-0 bg-[var(--nova-surface-2)] text-[var(--nova-text)]">
      ...
      />
      <LoreBatchTranslationDialog open={loreBatchTranslationOpen} onOpenChange={setLoreBatchTranslationOpen} workspace={workspace} items={items} />
      <MaterialImportDialog
        open={materialImportOpen}
        onOpenChange={setMaterialImportOpen}
        workspace={workspace}
        onImported={(ids) => {
          void refreshItems(ids[0])
          notifyLoreUpdated(ids)
        }}
      />
