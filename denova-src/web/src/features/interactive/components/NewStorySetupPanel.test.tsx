...
describe('NewStorySetupPanel', () => {
  ...
  it('defaults Narraverse imported new stories to manual director runs', async () => {
    const user = userEvent.setup()
    const onCreate = vi.fn().mockResolvedValue(undefined)
    render(<NewStorySetupPanel narraverseImported stories={[]} tellers={[]} directors={[director]} imagePresets={[]} onCancel={vi.fn()} onCreate={onCreate} />)

    expect(screen.getByRole('radio', { name: /仅手动/ })).toBeChecked()
    await user.click(screen.getByRole('button', { name: '继续选择开场方式' }))
    await waitFor(() => expect(onCreate).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('keeps an existing story director policy for Narraverse imports', () => {
    render(<NewStorySetupPanel narraverseImported stories={[]} story={{ id: 'st_1', title: '已有故事', origin: '', story_teller_id: 'classic', story_director_id: 'default', director_run_policy: { mode: 'interval', interval_turns: 5 }, choice_count: 5, reply_target_chars: 2000, opening: { mode: 'ai' }, created_at: '', updated_at: '', branches: 1, events: 0 }} tellers={[]} directors={[director]} imagePresets={[]} onCancel={vi.fn()} onCreate={vi.fn()} />)
    expect(screen.getByRole('radio', { name: /每 X 回合自动/ })).toBeChecked()
    expect(screen.getByLabelText('导演自动规划间隔')).toHaveValue(5)
  })
