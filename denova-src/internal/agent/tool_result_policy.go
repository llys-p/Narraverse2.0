...
func ManifestForTool(name string) ToolManifest {
	...
	case isMasterScopedTool(normalized):
		// Master tools persist only through their own Proposal/CAS/Risk path.
		// They are not filesystem writes and must not inherit the generic
		// create_*/write_* file_write heuristic.
...
func isMasterScopedTool(name string) bool {
	switch name {
	case "create_master_proposal", "validate_master_patch", "apply_master_patch":
		return true
	default:
		return false
	}
...
func isMasterReadTool(name string) bool {
	switch name {
	case "list_master_assets", "read_master_asset", "read_master_source", "read_master_field", "get_master_asset", "get_master_field", "get_master_pipeline_status", "get_master_translation_status", "get_master_issue":
		return true
	default:
		return false
	}
