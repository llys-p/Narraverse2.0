...
func TestConfigManagerToolsExposeMasterSchemaWhenLoreToolsAreEnabled(t *testing.T) {
	tools, err := newConfigManagerTools(&config.Config{Workspace: t.TempDir()}, config.ResolvedAgentToolSettings{LoreRead: true, LoreWrite: true})
	if err != nil {
		t.Fatal(err)
	}
	names := configManagerToolNameSet(t, tools)
	for _, name := range []string{"list_master_assets", "read_master_asset", "read_master_source", "read_master_field", "create_master_proposal", "validate_master_patch"} {
		if !names[name] {
			t.Fatalf("config manager should expose %s when lore tools are enabled, names=%v", name, names)
		}
	}
	if names["apply_master_patch"] {
		t.Fatal("config manager must not apply Master patches directly")
	}
}
