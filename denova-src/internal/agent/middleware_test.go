...
func TestMasterScopedToolsBypassFilesystemCapabilityHeuristic(t *testing.T) {
	for _, name := range []string{"create_master_proposal", "validate_master_patch", "apply_master_patch"} {
		manifest := ManifestForTool(name)
		if manifest.Capability != "" {
			t.Fatalf("%s capability = %q, want no generic capability", name, manifest.Capability)
		}
		...
		endpoint, err := middleware.WrapInvokableToolCall(
			...
			&adk.ToolContext{Name: name, CallID: "call-master"},
		)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !called || !strings.Contains(result, "ok") {
			t.Fatalf("%s should enter its controlled logic with file_write disabled: called=%v result=%q", name, called, result)
		}
	}
}

func TestMasterFixKeepsFilesystemWritesBlocked(t *testing.T) {
	middleware := &toolOrchestratorMiddleware{
		agentKind:           config.AgentKindToolAgent,
		enforceToolSettings: true,
		toolSettings:        config.ResolvedAgentToolSettings{},
	}
	for _, name := range []string{"create_file", "write_file"} {
		called := false
		endpoint, err := middleware.WrapInvokableToolCall(
			context.Background(),
			func(context.Context, string, ...tool.Option) (string, error) {
			...
			&adk.ToolContext{Name: name, CallID: "call-filesystem"},
		)
		if err != nil {
			t.Fatal(err)
		}
		...
		if called || !strings.Contains(result, "file_write") {
			t.Fatalf("%s should remain blocked: called=%v result=%q", name, called, result)
		}
	}
}
