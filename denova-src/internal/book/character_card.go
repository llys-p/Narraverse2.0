...
type tavernBookEntry struct {
	...
	SourceRecordID      string         `json:"-"`
	SourceRaw           map[string]any `json:"-"`
}

func (entry *tavernBookEntry) UnmarshalJSON(data []byte) error {
	type alias tavernBookEntry
	var decoded alias
	...
	*entry = tavernBookEntry(decoded)
	entry.SourceRaw = raw
	entry.SourceRecordID = masterSourceEntryIdentity(raw)
	return nil
}
