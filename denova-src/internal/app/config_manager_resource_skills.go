...
func configManagerResourceSkillNames(req ConfigManagerRequest) []string {
	...
	signals := []string{req.Instruction, req.Origin, req.ResourceID, req.StoryID, req.BranchID}
	...
	if isMasterLibrary {
		add(configManagerMasterLibrarySkill)
	} else if strings.Contains(text, "write_lore_items") || strings.Contains(text, "lore_item") || strings.Contains(text, "selected_lore") || strings.Contains(text, "资料库") {
		add(configManagerLoreSkill)
	}
	switch {
