...
| Pipeline usable / staging | CURRENT（usable / staging 的当前推导已经可用；parse / normalize / check 的部分节点状态仍依赖现有数据推导，因此这部分仍属于 PARTIAL） |
...
当前 5 秒稳定窗口与 8097 自身 retry 之间尚无协议级互斥保证：存在“窗口期内 8097 即将自行重试”的竞态可能，这是现有 terminal-failure 兼容逻辑的一部分技术债；本版本未修改 8097 核心队列。

因此，当前 `failed + 稳定约 5 秒` 的兼容判断，与 8097 自身可能即将发生的 retry 之间，尚没有协议级互斥保证。这仍属于 1.0 的临时 terminal-failure 兼容逻辑，长期应由 8097 提供明确的 `terminal_failure / retry_exhausted / next_retry` 等协议字段。
...
| `web/src/features/interactive/components/setting-panel/MaterialImportDialog.tsx` | Adventure 加载资料 |

> 附录 B 的路径均以 `denova-src/` 为代码根目录；Adventure 素材导入入口的完整相对路径为 `web/src/features/interactive/components/setting-panel/MaterialImportDialog.tsx`。
